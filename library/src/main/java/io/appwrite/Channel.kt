...
class Channel<out T> private constructor(
     ...
     */
    fun <N> next(segment: String, id: String? = null): Channel<N> {
        return if (id == null) {
            Channel(segments + segment)
        } else {
            Channel(segments + listOf(segment, normalize(id)))
        }
    }
 ...
 */
fun Channel<_Collection>.document(id: String? = null): Channel<_Document> =
 ...
 */
fun Channel<_Table>.row(id: String? = null): Channel<_Row> =
    this.next("rows", id)
 ...
 */
fun Channel<_Bucket>.file(id: String? = null): Channel<_File> =
    this.next("files", id)

// --- TERMINAL ACTIONS ---
// Restricted to Actionable types (_Document, _Row, _File, _Team, _Membership)
